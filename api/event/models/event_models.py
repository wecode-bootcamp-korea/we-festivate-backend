...
class EventPost(models.Model):
    ...
    place           = models.CharField(max_length = 50)                     # 이벤트 진행 상세 위치
    date            = models.CharField(max_length = 50, blank = True)       # 이벤트 진행날짜
    start_time      = models.CharField(max_length = 50, blank = True)       # 이벤트 시작시간
    end_time        = models.CharField(max_length = 50, blank = True)       # 이벤트 종료시간
    max_rsvp        = models.IntegerField(blank = True, default = 0)        # 최대 신청 허용 인원
    current_rsvp    = models.IntegerField(default = 0)                      # 현재 신청 인원
    rsvp_members    = models.ManyToManyField(
        User,
        through = 'EventRsvp',
        related_name = 'rsvp_members'
        )
